...
void lemonade::rmproduct(const name &product_name) {
    ...
    products products_table(get_self(), get_self().value);
    auto     productIdx = products_table.get_index<eosio::name("byname")>();
    auto     existing_product = productIdx.require_find(product_name.value,
...
void lemonade::stake(const name &owner, const asset &quantity, const name &product_name,
    ...
    if (existing_product->duration != 0) {
        ended_at = now() + secondsPerDay * existing_product->duration;
    }
    ...
    // If staking exist, update exist staking
        ...
        asset total_led_reward = asset(existing_staking->balance.amount * yield_per_sec
        ...
        if (existing_product->has_lem_rewards == true) {
            for (int i = 0; i < 3; i++) {
                if (existing_config->last_half_life_updated[i] <= current
                    ...
                    total_lem_reward_amount
                        += ((existing_config->last_half_life_updated[i + 1] - last_reward)
                            / (uint32_t)pow(2, i));
                    last_reward = existing_config->last_half_life_updated[i + 1];
                }
            }

            asset total_lem_reward = asset(existing_staking->balance.amount * total_lem_reward_amount
        ...
        if (to_owner_led.amount > 0) {
            action(permission_level { get_self(), "active"_n }, "led.token"_n, "transfer"_n,
                   make_tuple(get_self(), owner, to_owner_led, string("stake reward")))
                .send();
        }
        if (existing_product->has_lem_rewards == true && to_owner_lem.amount > 0) {
            action(permission_level { get_self(), "active"_n }, "led.token"_n, "transfer"_n,
                   make_tuple(get_self(), owner, to_owner_lem, string("stake reward")))
                .send();
        }
...
void lemonade::claimled(const name &owner, const name &product_name) {
    ...
    auto current = now();
    if (existing_product->duration != 0) {
        current = now() >= existing_staking->ended_at ? existing_staking->ended_at
                                                      : now();
    }

    const auto secs_since_last_reward = (current - existing_staking->last_claim_led_reward);

    const auto yield_per_sec = (existing_staking->current_yield - 1) / secondsPerYear;
    asset to_owner_led = asset(0, symbol("LED", 4));
    to_owner_led.amount
        = existing_staking->balance.amount * secs_since_last_reward * yield_per_sec;
...
void lemonade::claimlem(const name &owner, const name &product_name) {
    ...
    auto       rewards_end_time
        = current >= existing_staking->ended_at ? existing_staking->ended_at : current;
    ...
    for (int i = 0; i < 3; i++) {
        if (existing_config->last_half_life_updated[i] <= rewards_end_time
            ...
            amount += ((existing_config->last_half_life_updated[i + 1] - last_reward)
                       / (uint32_t)pow(2, i));
            last_reward = existing_config->last_half_life_updated[i + 1];
        }
    }
    ...
    asset to_owner_lem
        = asset(existing_staking->balance.amount * amount * lem_reward_rate / secondsPerHour,
                symbol("LEM", 4));
...
void lemonade::changeyield(const name &owner, const name &product_name,
    ...
    check(yield >= existing_product->minimum_yield && yield <= existing_product->maximum_yield,
...
void lemonade::createbet(const uint32_t &started_at, const uint32_t &betting_ended_at,
...
void lemonade::claimbet(const uint64_t &bet_id) {
    ...
    // Betting both side -> claim for winner
    {
        ...
        double                    dividend;
        string win_position = existing_betting->base_price >= existing_betting->final_price
                                  ? "short"
                                  : "long";
        ...
        for (auto k : winners) {
            const asset price
                = asset(k.second.amount + k.second.amount * losers_total / winners_total * betting_ratio,
                        k.second.symbol);
            if (price.amount == 0) {
                continue;
            }
...
void lemonade::issue_lem() {
    ...
    for (int i = half_life; i < 3; i++) {
        if (existing_config->last_half_life_updated[i] <= current
            ...
            secs_since_last_fill
                += ((existing_config->last_half_life_updated[i + 1] - last_issued)
                    / (uint32_t)pow(2, i));
    ...
    led_token_stats stats_table(led_token_contract, symbol("LEM", 4).code().raw());
    ...
    if (new_token.amount + existing_stats->supply.amount > LEM_MAX) {
        new_token = asset(LEM_MAX - existing_stats->supply.amount, symbol("LEM", 4));
    }
    ...
    if (new_token.amount > 0) {
        ...
        if (to_team.amount > 0) {
            action(permission_level { get_self(), "active"_n }, "led.token"_n, "transfer"_n,
                   make_tuple(get_self(), team_account, to_team, string("fill bucket to team")))
                .send();
        }
